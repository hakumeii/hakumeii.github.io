  ...
  const fields = ["#ope1","#ope2","#ope3", "#ope4","#ope5","#ope6","#ope7","#ope8","#ope9","#hhe1","#hhe2","#hhe3", "#hhe4","#hhe5","#hhe6","#hhe7","#hhe8","#hhe9","#lhe1","#lhe2","#lhe3", "#lhe4","#lhe5","#lhe6","#lhe7","#lhe8","#lhe9","#oru1","o#ru2","#oru3", "#oru4","#oru5","#oru6","#oru7","#oru8","#oru9"];
  ...
  function calculateIT() {
    ...
    //annihilation
    totalOrundum += parseInt($("#annihilation").val())/7*daysF;
    // mission calculation
    for (var i = 0; i < mission.length; i++) {
      if ($(mission[i].length)) {
        if ($(mission[i]).is(":checked")) {
          let temp = parseInt($(mission[i]).val())/7*daysF;
          totalOrundum+= temp;
        }
      }
    }
    ...
    for (var i = 0; i < certStore.length; i++) {
      if ($(certStore[i].length)) {
        if ($(certStore[i]).is(":checked")) {
          let temp = parseInt($(certStore[i]).val())/30*daysF;
          totalOrundum+= temp;
        }
      }
    }
